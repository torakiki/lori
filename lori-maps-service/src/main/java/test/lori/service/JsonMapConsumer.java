...
/**
 * Consumer for the json map resource adding the map to the {@link JsonMap}
 * 
 * @author Andrea Vacondio
 *
 */
@Component
class JsonMapConsumer implements MapResourceConsumer {

    private String removablePrefix;

    @Autowired
    public JsonMapConsumer(@Qualifier("removablePrefix") String removablePrefix) {
        this.removablePrefix = removablePrefix;
    }

    @Override
    public void accept(JsonMap m, ServletContextResource r) {
        m.jsonPath(toExternalPath(r, removablePrefix));
    }
